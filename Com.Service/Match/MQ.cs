...
public class MQ
{
    ...
    public void DepthChange(List<Orders> orders, List<Deal> deal, List<Orders> cancel)
    {
        if (orders.Count() > 0 || deal.Count() > 0 || cancel.Count() > 0)
        {
            ...
            this.model.mq_helper.MqTask(FactoryService.instance.GetMqOrderDeal(this.model.info.market), Encoding.UTF8.GetBytes(JsonConvert.SerializeObject((process.no, orders, deal, cancel))));
        }
